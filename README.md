...
[![FRP Version](https://img.shields.io/badge/frpc-v0.64.0-brightgreen.svg)](https://github.com/fatedier/frp)
...
**This add-on is built on `FRP v0.64.0`.** 
Due to breaking changes in FRP's configuration format (switching from `.ini` to `.toml`) and underlying protocol optimizations, **your Server-side (`frps`) MUST run a compatible version (v0.64.x is strictly recommended)**. Using an outdated server version will result in connection refused or protocol mismatch errors.

**本插件底层核心基于 `FRP v0.64.0` 构建。**
由于 FRP 官方不仅全面弃用了旧版的 `.ini` 格式，还在新版中对 QUIC 协议和多路复用机制进行了深度优化，**请务必确保您的云端服务端（`frps`）版本与本客户端保持兼容（强烈建议服务端同步升级至 v0.64.x）**。如果服务端版本过低，将直接导致“连接被拒绝”或协议不匹配等报错！

---

## 🌟 Features / 核心特性

- **Fully Optimized (v0.64+):** Utilizes the latest `.toml` configuration format and advanced multiplexing for ultimate stability.
...
- **底层性能优化：** 采用 FRP 0.64 最新协议，连接更稳、断线重连更快，且全面拥抱 TOML 格式。
